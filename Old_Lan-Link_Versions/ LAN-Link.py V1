...
"""
LAN-Link V1.0.0 - All-in-One Edition with Direct Client-to-Client P2P File Sharing
    ...
    def _handle_client(self, sock, addr):
        ...
        try:
            ...
            help_msg = (
                ...
                "Commands: /quit, /who, /realname, /share, /files, /download <id>"
            )
            ...
            for line in f_in:
                ...
                if text.startswith("/download "):
                    ...
                    if len(parts) == 2:
                        ...
                        if not owner_ip or not file_port:
                            ...
                            # Send direct connection peer info back to downloader
                ...
                if text.startswith("/kick "):
                    if sock != self.host_socket:
                        ...
                        continue
                    target_user = text[6:].strip().lower()
                    target_sock = None
                    target_name = ""
                    with self.lock:
                        for s, info in self.clients.items():
                            if info["username"].lower() == target_user:
                                target_sock = s
                    ...
                    if target_sock and target_sock != self.host_socket:
                        ...
                        sock.sendall(f"User '{target_user}' not found or cannot kick host.\n".encode("utf-8"))
                    continue
...
class ChatApp:
    def __init__(self, root):
        self.root = root
        self.root.title("LAN-Link V1.0.0")
        ...
        self.version_lbl = ttk.Label(footer_frame, text="LAN-Link V1.0.0", font=("", 9, "bold"), foreground="gray")
    ...
    def _start_direct_download(self, owner_ip, file_port, fid, filename):
        def run_download_window():
            ...
            def peer_download_task():
                try:
                    s = socket.create_connection((owner_ip, file_port), timeout=10)
