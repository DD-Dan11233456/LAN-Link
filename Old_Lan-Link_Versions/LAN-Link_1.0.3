...
"""
LAN-Link V1.0.3 - All-in-One Edition with Direct Client-to-Client P2P File Sharing
...
class RoomServer:
    ...
    def _handle_client(self, sock, addr):
        # Normalize IP (strip IPv6-mapped IPv4 prefix if present for robust Linux handling)
        client_ip = addr[0]
        ...
        try:
            ...
            with self.lock:
                # The very first connected client is guaranteed to be the host
                ...
                self.clients[sock] = {"username": username, "email": user_email, "ip": client_ip}
                was_member = True
            ...
            for line in f_in:
                ...
                if text.startswith("/ban "):
                    ...
                    if target_sock and target_sock != self.host_socket:
                        try:
                            target_sock.sendall(b"REJECT:You have been banned from the room.\n")
                            target_sock.close()
                        except OSError:
                            pass
...
class ChatApp:
    def __init__(self, root):
        self.root = root
        self.root.title("LAN-Link V1.0.3")
        ...
        # Direct P2P File Server Listener Setup (Dual-stack robust for Linux/Windows)
        ...
        self.version_lbl = ttk.Label(footer_frame, text="LAN-Link V1.0.3", font=("", 9, "bold"), foreground="gray")
